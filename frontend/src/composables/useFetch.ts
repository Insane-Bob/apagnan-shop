...
export function useFetch(url : ComputedRef , query = null, onSuccess = null, onError = null){
    const _path = computed(()=>{
        return url.value + (query ? `?${query}` : '')
    })
    async function _fetch(method : string, config : AxiosRequestConfig) :  Promise<any | void> {
        try{
