...
const removeFromCart = async () => {
    if (user.isAuthenticated && props.product) {
        const reponse = await apiClient.delete(
            'users/' + user.getId + '/basket/' + props.product.id,
        )
...
<template>
    <div v-if="quantity" class="flex items-center gap-x-4 my-4">
        ...
        <div v-else class="relative group min-w-20 h-20">
            ...
            />
            <div
                class="absolute invisible group-hover:visible bg-black/40 w-20 h-20 top-0 left-0 cursor-default flex items-center"
            >
        ...
        <div class="flex justify-end grow">
            <div class="flex flex-col mr-2 justify-end gap-y-2">
                <div class="flex items-center gap-x-2 max-w-72">
                    ...
                    >
                        <SelectTrigger
                            class="focus:ring-none ring-transparent focus:border-primary"
                        >
                            <SelectValue placeholder="Quantité" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem
                                v-for="(value, index) in product.stock +
                                parseInt(quantity)"
                                :value="value.toFixed()"
                                :key="index"
                            >
