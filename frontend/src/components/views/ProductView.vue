...
function streamStock() {
    ...
    stockSource = new EventSource(
        `${import.meta.env.VITE_API_BASE_URL}/products/${product.value?.slug}/stock`,
    )
...
const { items: suggestions, fetch: fetchSuggestions } = useSuggestion<Product>(
    [route?.params?.pslug],
    5,
    'products',
)
...
const fetchProduct = async () => {
    try {
        const response = await apiClient.get(
            'products/' + route.params.pslug + '?withImages=true',
        )
        ...
        if (data.product.images.length > 0) {
            ...
            carouselImages.value = ['/images/noPhotoAvailable.webp']
        }
    } catch (e) {
        toast({
...
const fetchCollection = async () => {
    try {
        ...
        if (data.collection.image) {
            ...
            collectionImage.value = '/images/noPhotoAvailable.webp'
        }
    } catch (e) {
        toast({
    ...
    <loader :wait-for="product">
        <div>
            ...
            <div v-if="!loading && product" class="flex justify-center">
                <div
                    class="w-4/5 flex flex-col md:flex-row justify-center gap-16 mb-16"
                >
                    ...
                    <div class="w-full md:w-1/2 flex flex-col gap-10">
                        ...
                        <div>
                            ...
                            >
                                {{
                                    stock > 0
                                        ? 'Il reste ' + stock + ' article(s)'
                                        : "Il n'y a plus de stock, revenez plus tard"
                                }}
                            </p>
                            <p v-else class="text-green-700">En stock</p>
                        </div>
            ...
            >
                <div class="w-3/5 flex flex-col items-center">
                    ...
                    <div class="flex flex-col gap-4 text-left">
                        <ReviewDetailComponent :reviews="reviews" />
                        <div
            ...
            <SuggestionCarousel
                :suggestions="
                    suggestions.map((p: Product) => ({
                        ...p,
                        url: `/collections/${p?.Collection?.slug}/products/${p.slug}`,
                    }))
                "
            >
