        ...
        try {
            const response = await apiClient.post('/uploads', form, {
                ...
                onUploadProgress: (progressEvent) => {
                    images[index].uploadProgress = Math.round(
...
<template>
    <div>
        ...
        <div
            class="grid gap-8 my-4"
            :class="[props.multiple ? 'grid-cols-3' : 'grid-cols-1']"
        >
            <div
                v-for="(image, index) in images"
                :key="'image-' + index"
                class="col-span-1 flex items-stretch gap-2 bg-slate-50 p-2 border-2 border-dashed rounded"
            >
                <div
                    class="w-full items-center h-full min-h-[100px] relative grid"
                    v-if="!image.api"
                >
                    <Label for="picture" class="mb-3 inline-block"
                        >Image N°{{ index + 1 }}
                        <span class="uppercase text-xs text-slate-500">
                            {{
                                image.uploadProgress == -2
                    ...
                    <div v-if="image.preview">
                        <img
                            :src="image.preview"
                            alt="img"
                            class="h-[100px] w-[100px] object-contain object-center p-2"
                        />
                    </div>
                    <template v-else>
                        <input
                            ...
                            @input="(e) => handleInputFile(e, index)"
                            class="opacity-0 absolute top-0 left-0 right-0 bottom-0 cursor-pointer"
                        />
                        <OutlinedInput
                            id="picture"
                            type="file"
                            class="hidden mt-[60px]"
                        />
                        <ion-icon
                            name="cloud-upload-outline"
                            class="text-[3em] pointer-events-none text-slate-500 relative left-[50%] top-[-10%] translate-x-0.5 -translate-y-0.5"
                        ></ion-icon>
                    </template>
                    <span
                        v-if="
                ...
                <div v-else>
                    <Label for="picture" class="mb-3 inline-block"
                        >Image N°{{ index + 1 }}</Label
                    >
                    <img
                        :src="image.api.url"
                        alt="img"
                        class="h-[100px] w-[100px] object-contain object-center bg-white rounded p-2"
                    />
                </div>
                <Button
                    size="icon"
