...
const confirmAction = async () => {
  try {
    ...
    switch(e.status){
      case(500):
      error.value = 'Une erreur est survenue, Nous tentons de la résoudre, veuillez réessayer plus tard';
      break;
      case (401):
      error.value = 'Vous n\'avez pas les droits pour effectuer cette action';
      break;
      default:
    ...
    <AlertDialog v-model:open="open">
      ...
      <AlertDialogContent class="w-[30vw] px-12 py-8">
        ...
        <AlertDialogFooter class="flex items-center sm:justify-end w-full " :class="styleFooter">
          ...
          <Button v-if="!loading && !error" :class="styleConfirm" @click.prevent="confirmAction">Continue</Button>
          <Button v-if="loading" class="bg-gray-300 cursor-not-allowed">Chargement...</Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </template>