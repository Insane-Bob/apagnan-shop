...
import type { UserNotificationPreferencesObject } from '@/types/UserNotificationPreferences'
        ...
        actions: {
            ...
            async fetchPreferences() {
                ...
                const response = await apiClient.get(
                    `/users/${user.get.id}/notifications`,
                )
                    ...
                    return [
                        {
                            ...
                            label: this.isActivate(
                                modelId,
                                NotificationSubscriptionType.PRODUCT_RESTOCK,
                            )
                                ? 'Desactiver la notification de réapprovisionnement'
                                : 'Activer la notification de réapprovisionnement',
                            icon: this.isActivate(
                                modelId,
                                NotificationSubscriptionType.PRODUCT_RESTOCK,
                            )
                                ? 'notifications-off'
                        ...
                        },
                        {
                            ...
                            label: this.isActivate(
                                ...
                                ? 'Desactiver la notification de changement de prix'
                                : 'Activer la notification de changement de prix',
                            icon: this.isActivate(
                                modelId,
                                NotificationSubscriptionType.PRODUCT_PRICE_CHANGE,
                            )
                    ...
                    return [
                        {
                            ...
                            label: this.isActivate(
                                ...
                                ? 'Desactiver la notification de nouveaux produits'
                                : 'Activer la notification de nouveaux produits',
                            icon: this.isActivate(
                                modelId,
                                NotificationSubscriptionType.NEW_PRODUCT,
                            )
