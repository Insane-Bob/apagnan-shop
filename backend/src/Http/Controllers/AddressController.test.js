...
describe('BillingAddressController test routes', () => {
    ...
    test('GET /api/addresses or GET /api/users/:userId/address - index billing address (admin only)', async () => {
        await emptyTables()
        const admin = await UserFactory.withCustomer().create({ role: USER_ROLES.ADMIN })
        const ADDRESSES_ADMIN_COUNT = 4
        await BillingAddressFactory.count(ADDRESSES_ADMIN_COUNT).create({ customerId: admin.customer.id})

        const user = await UserFactory.withCustomer().create()
        const ADDRESSES_USER_COUNT = 4
        await BillingAddressFactory.count(ADDRESSES_USER_COUNT).create({ customerId: user.customer.id})


        actingAs(admin)
        let response = await request(app).get('/api/addresses')
        expect(response.statusCode).toBe(200)
        expect(response.body).toHaveLength(ADDRESSES_ADMIN_COUNT + ADDRESSES_USER_COUNT)
